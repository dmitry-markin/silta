...
9. Install Claude Code under the `claude` system user (already created when installing `silta_*.deb` above):
   ```bash
   sudo runuser -u claude -- bash -c 'curl -fsSL https://claude.ai/install.sh | bash'
   ```
...
13. Add exactly one Claude auth token as `/etc/silta/auth/user-1/oauth-token` (for a personal session using Claude subscription, generated by `claude setup-token`) or `/etc/silta/auth/user-1/api-key` (for a session using Claude Platform API key). The file must be owned by root with 0600 permissions.
